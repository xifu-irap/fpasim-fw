...
class VunitConf(VunitUtils):
    ...
    def main(self):
        ...
        fct_name = self.class_name + '.main'
        if self.verbosity > 0:
            str0 = fct_name
            display_obj.display_title(msg_p=str0, level_p=level0)

        def convert_ucdb_to_xml(input_file_path_p,output_file_path_p):
            """
            This function convert an input coverage file (*.ucdb) into an output coverage xml file.
            ...
            fct_name = self.class_name + '.main' + '.convert_ucdb_to_xml'
            if self.verbosity > 0:
                str0 = fct_name
                display_obj.display_title(msg_p=str0, level_p=level1)
        ...
        def convert_absolute_to_relative_path(input_file_path_p,output_file_path_p):
            ...
            -------
                None
            """
            fct_name = self.class_name + '.main' + '.convert_absolute_to_relative_path'
            if self.verbosity > 0:
                str0 = fct_name
                display_obj.display_title(msg_p=str0, level_p=level1)
            ...
            if self.verbosity > 0:
                str0 = "input_file_path_p: " + input_file_path_p
                display_obj.display(msg_p=str0, level_p=level2)
                str0 = "output_file_path_p: " + output_file_path_p
                display_obj.display(msg_p=str0, level_p=level2)
        ...
        def post_run(results):
            ...
            """
            name,index = self.json_key_path.split('/')


            ########################## Generate a single *.ucdb file by test
            # Unique filename by test (Each test can have 1 or several test variants)           
            filename = "coverage_"+name+"_"+index
            # temporary directory
            tmp_base_path = str(Path(self.base_path_dic["results_path"],"tmp"))

            self.create_directory(path_p=tmp_base_path, level_p=level1)
                        
            # Temporary output coverage filepath (one by test) 
            merge_filepath_tmp = str(Path(tmp_base_path,filename + ".ucdb"))

            # For the processed test, merge the coverage files generated by the test variant(s)
            #   The list of files to process are defined in the Vunit auto-generated "coverage_files.txt"
            results.merge_coverage(file_name=merge_filepath_tmp)
            
            ########################## Conversion *.ucdb -> *.xml
                 # output coverage filepath
            output_merge_filepath = merge_filepath_tmp
            # output xml filepath
            output_merge_file_xml = str(Path(self.base_path_dic["results_path"],filename + ".xml"))

            # search all temporary coverage files
            search_file_obj = FilepathListBuilder()
            search_file_obj.set_file_extension([".ucdb"])
            search_file_obj.add_basepath(basepath_p=tmp_base_path)
            filepath_list = search_file_obj.get_filepath_list()

            # convert the output coverage file into the output coverage xml file
            convert_ucdb_to_xml(input_file_path_p=output_merge_filepath,output_file_path_p=output_merge_file_xml)

            # Post file processing
            # for all file path defined in the output coverage xml file, replace the absolute path by its relative path from the git_root_path
            convert_absolute_to_relative_path(input_file_path_p=output_merge_file_xml,output_file_path_p=output_merge_file_xml)
