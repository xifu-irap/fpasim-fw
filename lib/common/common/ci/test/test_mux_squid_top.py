...
def test_mux_squid_top():
    ...
    # result
    output_filepath_in = str(Path(output_base_path,"mux_squid_in.csv"))
    output_filepath_out = str(Path(output_base_path,"mux_squid_out.csv"))
    ...
    obj_attr.set_attribute(name_p="adc_mux_squid_feedback", mode_p=0, min_value_p=1, max_value_p=10)
    pts_list = obj_attr.run()

    obj_attr = Attribute(pts_list_p=pts_list)
    obj_attr.set_random_seed(value_p=10)
    obj_attr.set_attribute(name_p="tes_out", mode_p=1, min_value_p=10, max_value_p=20)
    pts_list = obj_attr.run()
